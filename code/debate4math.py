...
class Debate:
    def __init__(self,
        ...
        Args:
            model_name (str): anthropic model name
            temperature (float): higher values make the output more random, while lower values make it more focused and deterministic
        ...
        if self.save_file['base_answer'] == "":
            # print("entered save_file")
            self.create_base()

        # creat&init agents
        # print("before create_agents")
        self.creat_agents()
        # print("after create_agents")
        # print("before init_agents")
        self.init_agents()
        # print("after init_agents")


    def init_prompt(self):
        def prompt_replace(key):
            self.save_file[key] = self.save_file[key].replace("##debate_topic##", self.save_file["debate_topic"]).replace("##base_answer##", self.save_file["base_answer"])
    ...
    def create_base(self):
        print(f"\n===== Counterintuitive Math Logic Task =====\n{self.save_file['debate_topic']}\n")
        ...
        agent.add_event(self.save_file['debate_topic'])
        # print("added event")
    ...
    def init_agents(self):
        ...
        print("MODERATOR ANSWER", self.mod_ans)
    ...
    def run(self):
        ...
        if self.mod_ans["debate_answer"] != '':
            ...
            with open("data/CounterintuitiveQA/raw_output_sonnet.txt", 'a') as file:
                file.write(str(self.mod_ans["debate_answer"]) + "\n")
...
def parse_args():
    ...
    parser.add_argument("-m", "--model-name", type=str, default="claude-sonnet-4-5-20250929", help="Model name")
...
if __name__ == "__main__":
    ...
    for id, input in enumerate(tqdm(inputs)):
        ...
        config['debate_topic'] = input.split('\t')[0]
